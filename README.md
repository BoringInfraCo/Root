...
- **MCP** — `root mcp serve` is a stdio shim to the local `rootd` Unix socket. `root connector` installs local packages. `root event` records inbound mail without starting an agent. `root checkpoint-sync` encrypts checkpoint references between paired installs. `root sync` still reconciles the Nix profile.
