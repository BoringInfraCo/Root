...
- **MCP** — `root mcp serve` is a stdio shim to the local `rootd` Unix socket. `root mcp daemon --http 127.0.0.1:PORT` adds loopback Streamable HTTP with a bearer token. `root connector` installs local packages. `root event` records inbound mail and explicit routes without starting an agent. Write and send wait on `root approval`.
